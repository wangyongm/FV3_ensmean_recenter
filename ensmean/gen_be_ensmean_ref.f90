...
!----------------------------------------------------------------------
   ...
   real(8), allocatable     :: data_r(:,:,:)             ! Data array.
   real(8), allocatable     :: data_r_mean(:,:,:)        ! Data array mean.
...
!  Calculate accumulating mean and variance:
   ...
   call mpi_allreduce(data_r,data_r_mean,nsize,mpi_double,mpi_sum,new_comm,iret)

   print*,maxval(data_r),minval(data_r)
